...
#Feature Generation
x$var1 <- as.integer((20*x$mvar1+5*x$mvar11+4*x$mvar16+15*x$mvar21))
x$var2 <- as.integer((20*x$mvar2+5*x$mvar12+4*x$mvar17+15*x$mvar22))
x$var3 <- as.integer((20*x$mvar3+5*x$mvar13+4*x$mvar18+15*x$mvar23))
x$var4 <- as.integer((20*x$mvar4+5*x$mvar14+4*x$mvar19+15*x$mvar24))
x$var5 <- as.integer((20*x$mvar5+5*x$mvar15+4*x$mvar20+15*x$mvar25))
...
testing$var1 <- as.integer((20*testing$mvar1+5*testing$mvar11+4*testing$mvar16+15*testing$mvar21))
testing$var2 <- as.integer((20*testing$mvar2+5*testing$mvar12+4*testing$mvar17+15*testing$mvar22))
testing$var3 <- as.integer((20*testing$mvar3+5*testing$mvar13+4*testing$mvar18+15*testing$mvar23))
testing$var4 <- as.integer((20*testing$mvar4+5*testing$mvar14+4*testing$mvar19+15*testing$mvar24))
testing$var5 <- as.integer((20*testing$mvar5+5*testing$mvar15+4*testing$mvar20+15*testing$mvar25))
...
x2[,c(1,29:33,39:45,48:50):=NULL]
testing2[,c(1,29:33,39:44,47:49):=NULL]
# x2[,c(12:26,34:38,44:48,53,54,49,50):=NULL]
# testing2[,c(12:26,34:38,44:48,53,52,49):=NULL]
x2$target <- y
x3 <- sample_n(x2,12500)
...
probability_table2<-as.data.table((matrix(prediction1,nrow = 3056,byrow = T)))
...
sol_copy <- copy(sol)
pp <- copy(p)
write.csv(sol,"twist1.csv",row.names = FALSE)
...
x2 <- copy(x)
testing2<- copy(testing)
x2[,c(29:33,39:45,48:50):=NULL]
testing2[,c(29:33,39:44,47:49):=NULL]
# x2[,c(12:26,34:38,44:48,53,54,49,50):=NULL]
# testing2[,c(12:26,34:38,44:48,53,52,49):=NULL]
x2$target <- y
x3 <- sample_n(x2,3000)
...
prediction1<-predict(model_xgb2,testing2, missing=NA)
maxn <- function(n) function(x) order(x, decreasing = TRUE)[n]
probability_table2<-as.data.table((matrix(prediction1,nrow = 20978,byrow = T)))
p <- as.data.frame(probability_table2)
p[, "max1"] <- apply(p[, 1:5], 1, function(x)x[maxn(1)(x)])
p[, "pos1"] <- apply(p[, 1:5], 1, maxn(1))
# p[, "max2"] <- apply(p[, 1:5], 1, function(x)x[maxn(2)(x)])
...
write.csv(sol,"second1.csv",row.names = FALSE)
...
sol$actual_vote <- NA
sol$actual_vote[which(pp$max1 - pp$max2 >= 0.15 )] <- sol_copy$actual_vote[which(pp$max1 - pp$max2 >= 0.15 )]
sol$actual_vote[which(is.na(sol$actual_vote ))] <- sol_no_history$actual_vote[which(is.na(sol$actual_vote))]

#FINAL ANSWER
write.csv(sol,"third1.csv",row.names = FALSE)
