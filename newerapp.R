...
table$y4<- 0.1
table$y5<- 0.1
table$y1[table$y == 1 ] <- 0.6
table$y2[table$y == 2 ] <- 0.6
table$y3[table$y == 3 ] <- 0.6
table$y4[table$y == 4 ] <- 0.6
table$y5[table$y == 5 ] <- 0.6
...
prob <- readRDS("prob2.rds")
testing$var16 <- prob$V1
testing$var17 <- prob$V2
testing$var18 <- prob$V3
testing$var19 <- prob$V4
testing$var20 <- prob$V5
...
ans$ans[ans$ans == 2 ] <- "Centaur"
ans$ans[ans$ans == 3 ] <- "Cosmos"
ans$ans[ans$ans == 4 ] <- "Ebony"
ans$ans[ans$ans == 5 ] <- "Odyssey"
ans$ans[ans$ans == 1 ] <- "Tokugawa"
...
sol[,sol:=NULL]
sol$actual_vote <- ans
write.csv(sol,"StockUp_IITKGP_33_xgb2ice_.csv",row.names = FALSE)
